...
function renderRecommendationPositionPerformance(){
  ...
  const rows=(perf.top20||perf.positions?.slice(0,20)||perf.top10||[]);const rate=v=>Number.isFinite(Number(v))?`${fmt(v,1)}%`:'—';
  const bestCount=perf.bestByTargetCount;const bestRate=perf.bestByTargetRate;
  const rangeTo=Number(perf.comparisonRange?.to||20);
  panel.innerHTML=`<div class="panel-head"><div><div class="eyebrow">RECOMMENDATION NUMBER PERFORMANCE</div><h2>أي رقم توصية يحقق الهدف أكثر؟</h2><p>Future-only من <b>${esc(perf.trackingStartsOn)}</b>. الرقم هو ترتيب التوصية وقت إصدارها داخل قائمة V18 اليومية.</p></div><span class="pill">Rank 1–${fmt(rangeTo,0)}</span></div>
  ...
  <div class="performance-note"><b>مهم:</b> المقارنة الآن تشمل التوصيات من #1 إلى #${fmt(rangeTo,0)}. «الأكثر تحقيقًا للهدف» بالعدد وحده لا يكفي، لذلك نعرض أيضًا Target% من الصفقات المحسومة، ولا نعتبر ترتيبًا متفوقًا رسميًا قبل ${fmt(perf.rankingMinimumResolvedSignals,0)} نتائج محسومة.</div>
