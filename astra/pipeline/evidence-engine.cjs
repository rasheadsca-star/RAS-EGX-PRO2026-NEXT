...
function buildEvidence(signal,row,regime,decisionInputSnapshotId){
  ...
  const items=[
    ...
    evidence(base,'LIQUIDITY','TURNOVER20',technical.sourceRef,{turnover20Egp:finite(signal.sourceCandidate.turnover20,0),passed:finite(signal.sourceCandidate.turnover20,0)>=1e6},'MARKET_DATA',[]),
