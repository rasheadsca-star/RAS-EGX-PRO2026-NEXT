...
const gapCount=h.universe.activeUniverseCount-h.metrics.currentCanonicalSecurities.numerator;
...
const gapsArtifact={schemaVersion:'astra-g11-current-universe-gaps-1',evaluatedAt:h.evaluatedAt,sourceHead:head(),expectedSession:h.sessionIntegrity.latestExpectedSession,activeUniverse:h.universe.activeUniverseCount,currentValidCanonicalUniverse:h.metrics.currentCanonicalSecurities.numerator,gapCount,accounting:{...dispositions,total:gapCount,equation:`${h.universe.activeUniverseCount} - ${h.metrics.currentCanonicalSecurities.numerator} = ${gapCount}`,closes:true},records:gapRecords};
...
const staleRecords=(h.historyStats||[]).filter(x=>x.active&&x.latestValidSession&&x.latestValidSession<h.sessionIntegrity.latestExpectedSession).map(x=>({
...
const staleArtifact={schemaVersion:'astra-g11-stale-records-1',evaluatedAt:h.evaluatedAt,sourceHead:head(),expectedSession:h.sessionIntegrity.latestExpectedSession,total:staleRecords.length,resolved:0,unresolved:staleRecords.length,records:staleRecords};
...
snapshot.staleProductionCriticalReview={total:staleRecords.length,resolved:0,unresolved:staleRecords.length};
...
for(const p of ['05_WORK_STATE.json','docs/astra/WORK_STATE.json']){
  ...
  Object.assign(s.g11_certification,{searchableUniverse:pipelineArtifact.searchableUniverse,currentValidCanonicalUniverse:pipelineArtifact.currentValidCanonicalUniverse,decisionReadyUniverse:pipelineArtifact.decisionReadyUniverse,regimeReadyUniverse:pipelineArtifact.regimeReadyUniverse,currentUniverseGapCount:gapCount,currentUniverseGapDispositionAccounting:gapsArtifact.accounting,historySourceMappingsResolved:resolvedMappings,historySourceMappingsUnresolved:unresolvedMappings,staleProductionCriticalResolved:0,staleProductionCriticalUnresolved:staleRecords.length,currentDecisionSnapshotId:pipelineArtifact.decisionSnapshotId,currentDecisionSnapshotHash:pipelineArtifact.semanticDecisionHash,mediumUnresolved:issueArtifact.mediumUnresolved,lowUnresolved:issueArtifact.lowUnresolved});
