...
function normalizeEvidence(e){
  ...
  return {ticker,reutersCode:canonicalReuters(ticker),isin,evidenceType,sourceId,effectiveDate,capturedAt,segment:SEGMENTS.has(segment)?segment:'UNKNOWN',companyName:e.companyName??null,status:String(e.status??'ACTIVE').toUpperCase(),sourceUrl:e.sourceUrl??null,sourceHash:e.sourceHash??null,sourceReceiptHash:e.sourceReceiptHash??null,scopeProofHash:e.scopeProofHash??null,universeCertificateHash:e.universeCertificateHash??null};
}
function inclusionProofValid(e){return !INCLUSION_EVIDENCE.has(e.evidenceType)||(HEX64.test(String(e.sourceReceiptHash??''))&&HEX64.test(String(e.scopeProofHash??''))&&HEX64.test(String(e.universeCertificateHash??'')))}
...
export function buildAuthoritativeUniverse(evidence,{asOfDate,declaredTotal=null,requireSnapshot=true}={}){
  ...
  const canonical=[...activeTickers].map(ticker=>{const items=byTicker.get(ticker)??[],inclusion=items.filter(x=>INCLUSION_EVIDENCE.has(x.evidenceType)&&x.status!=='DELISTED'),best=[...inclusion,...items].sort((a,b)=>(b.effectiveDate??'').localeCompare(a.effectiveDate??'')||a.sourceId.localeCompare(b.sourceId))[0];const sourceEvidence=items.map(x=>({sourceId:x.sourceId,evidenceType:x.evidenceType,effectiveDate:x.effectiveDate,capturedAt:x.capturedAt,sourceUrl:x.sourceUrl,sourceHash:x.sourceHash,sourceReceiptHash:x.sourceReceiptHash,scopeProofHash:x.scopeProofHash,universeCertificateHash:x.universeCertificateHash})).sort((a,b)=>JSON.stringify(a).localeCompare(JSON.stringify(b)));return {ticker,reutersCode:`${ticker}.CA`,isin:best.isin??items.find(x=>x.isin)?.isin??null,companyName:best.companyName??items.find(x=>x.companyName)?.companyName??null,segment:best.segment??'UNKNOWN',status:'ACTIVE',identityVerified:items.some(x=>IDENTITY_EVIDENCE.has(x.evidenceType)),inclusionVerified:inclusion.length>0,evidenceCount:items.length,evidenceHash:sha256(sourceEvidence)}}).sort((a,b)=>a.ticker.localeCompare(b.ticker));
  ...
  const sourceManifest={asOfDate,sources:[...new Set(rows.map(x=>x.sourceId))].sort(),evidenceTypes:[...new Set(rows.map(x=>x.evidenceType))].sort(),evidenceCount:rows.length,declaredTotal,sourceReceiptHashes:[...new Set(snapshotRows.map(x=>x.sourceReceiptHash))].sort(),scopeProofHashes:[...new Set(snapshotRows.map(x=>x.scopeProofHash))].sort(),universeCertificateHashes:[...new Set(snapshotRows.map(x=>x.universeCertificateHash))].sort()};sourceManifest.manifestHash=sha256(sourceManifest);
