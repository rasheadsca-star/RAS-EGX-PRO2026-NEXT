(()=>{
  ...
  function render(){
    ...
    verdict.innerHTML=`<b>${esc(d.stageAr||'')}</b><span class="prv2-hard-gate ${hardGatePass?'pass':'block'}">${hardGatePass?'Hard Gates PASS':'Hard Gate غير مكتمل'}</span><br>${d.professionalClaimAllowed?'اجتاز شروط الجاهزية المهنية المعرفة في V2.':'الأساس قوي، لكن لا يُسمح بادعاء الاعتماد المهني حتى اكتمال البوابات الإلزامية.'}`;
