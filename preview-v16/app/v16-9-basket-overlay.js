...
(() => {
  ...
  function card(row, sessionAligned, executionEligible) {
    ...
    const weightLabel = executionEligible
      ...
      : sessionAligned
        ? '0% تنفيذ — الخطة محفوظة'
        : 'مرجع فقط — اختلاف جلسة';
    const rule = executionEligible
      ? (row.morningConfirmation?.ruleAr || 'التنفيذ معلق على افتتاح داخل النطاق وسيولة مؤكدة خلال أول 10–15 دقيقة.')
      : sessionAligned
        ? 'السلة متزامنة مع آخر جلسة مكتملة، لكن التنفيذ مغلق لأن آخر تحقق للمصدر لم يحقق Execution Grade. لا شراء حتى إعادة فتح بوابة المصدر.'
        : 'هذه الخطة معروضة كمرجع فقط لأن جلسة التوصية لا تطابق آخر جلسة سوق موثقة. لا تنفيذ حتى إعادة بناء السلة بنفس محرك V16.9.';
  ...
  function render(decision, priceTruth) {
    ...
    const executionEligible = Boolean(sessionAligned && executionGrade && decision?.practicalReady === true && rows.length > 0);
    ...
    const badge = !sessionAligned
      ...
      : executionEligible
        ? 'متزامنة — تأكيد الافتتاح مطلوب'
        : 'متزامنة — التنفيذ مغلق مؤقتًا';
    ...
    const truthDetail = !sessionAligned
      ...
      : executionEligible
        ...
        : `الخطة مخصصة لأول جلسة تداول تالية: ${friendlySession(targetSession)}، لكن آخر محاولة مصدر لم تحقق Execution Grade؛ لذلك التعرض التنفيذي 0% حتى تحقق جديد موثوق.`;

    const exposureText = executionEligible
      ? `${fmt(plan.totalAllocationPct, 1)}%`
      : `0% تنفيذ${num(plan.totalAllocationPct) !== null ? ` · الخطة ${fmt(plan.totalAllocationPct, 1)}%` : ''}`;
    const cashText = executionEligible ? `${fmt(plan.cashReservePct, 1)}%` : '100% نقد تنفيذي';

    panel.innerHTML = `
      <div class="v169-head">
        <div>
          <h2>${esc(title)}</h2>
          <p>نفس محرك V16.9 ونفس أسلوب الاختيار والأوزان؛ التغيير يخص حقيقة الجلسة وبوابة التنفيذ فقط.</p>
        </div>
      ...
      <div class="v169-summary">
        ...
        <div><small>إجمالي التعرض</small><b>${esc(exposureText)}</b></div>
        <div><small>السيولة المحتفظ بها</small><b>${esc(cashText)}</b></div>
      </div>
