...
(() => {
    ...
    return `
      <article class="v169-card${hot ? ' hot' : ''}">
        ...
        <div class="v169-metrics">
          <div class="v169-metric">
            <span>الدخول من</span>
            <b>${fmt(row.entryLow, 4)}</b>
          </div>
          <div class="v169-metric">
            <span>الدخول إلى</span>
            <b>${fmt(row.entryHigh, 4)}</b>
          </div>
          <div class="v169-metric">
            <span>الهدف</span>
            <b>${fmt(row.target1, 4)}</b>
          </div>
          <div class="v169-metric">
            <span>الوقف</span>
            <b>${fmt(row.stopLoss, 4)}</b>
          </div>
          <div class="v169-metric">
            <span>RSI</span>
            <b>${fmt(row.rsi14, 1)}</b>
          ...
          <div class="v169-metric">
            ...
            <b>${fmt(row.volumeRatio20, 2)}×</b>
          </div>
        </div>

        <div class="v169-rule">
          ${esc(
            row.morningConfirmation?.ruleAr ||
            'التنفيذ معلق على افتتاح داخل النطاق وسيولة مؤكدة خلال أول 10–15 دقيقة.'
          )}
        </div>
      </article>`;
  }
  ...
  function render(decision) {
    ...
    const rows = Array.isArray(decision?.recommendations)
    ...
    if (!rows.length) {
      panel.innerHTML = `
        <div class="v169-error">
          ملف V16.9 لم يُرجع أعضاء سلة حاليًا. هذه الرسالة لا تخفي باقي نوافذ التطبيق.
        </div>`;
      return true;
    }
    ...
    panel.innerHTML = `
      <div class="v169-head">
        <div>
          <h2>سلة V16.9 الحالية</h2>
          <p>
            النتائج الجديدة داخل التطبيق الكامل؛ نوافذ بحث السوق والمحفظة والتحليل المالي والسجل الحي لم تُحذف.
          </p>
        </div>
      ...
      <div class="v169-summary">
        <div>
          <small>عدد الأسهم</small>
          <b>${rows.length}</b>
        </div>
        <div>
          <small>إجمالي التعرض</small>
          <b>${fmt(plan.totalAllocationPct, 1)}%</b>
        </div>
        <div>
          <small>السيولة المحتفظ بها</small>
          <b>${fmt(plan.cashReservePct, 1)}%</b>
        </div>
        <div>
          <small>مدة الخطة</small>
          <b>${fmt(plan.holdingSessions, 0)} جلسة</b>
        </div>
      </div>
    ...
    if (legacyPanel && legacyPanel !== panel) {
  ...
  async function start() {
    try {
      ...
      const apply = () => {
        if (!render(decision) && attempts++ < 50) {
