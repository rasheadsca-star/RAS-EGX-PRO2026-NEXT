...
function alternateValidPublicationSameSession(){const p=publication(),body=structuredClone(p);delete body.publicationHash;body.recommendations[0].qualityScore=93;body.recommendations[0].planHash='d'.repeat(64);return {...body,publicationHash:sha256(body)}}
...
test('new publication is hash chained and cannot be rewritten',()=>{let l=createForwardShadowLedger({startAfterSession:'2026-08-31'});l=appendPublicationToForwardShadowLedger(l,{publication:publication(),regime:regime()});assert.equal(l.entries.length,1);assert.ok(verifyForwardShadowLedger(l));assert.match(l.entries[0].entryHash,/^[a-f0-9]{64}$/);assert.throws(()=>appendPublicationToForwardShadowLedger(l,{publication:alternateValidPublicationSameSession(),regime:regime()}),/SHADOW_SESSION_ALREADY_FROZEN/)});
