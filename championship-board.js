...
function renderHTML(c){
  ...
  return `<div class="egx-champ">
    ...
    <div class="egx-score"><div class="egx-fighter"><strong>EGX ONE</strong><div class="pts">${c.score.current}</div><small>${esc(c.current.evidenceGrade)}</small></div><div class="egx-vs">VS</div><div class="egx-fighter"><strong>V16.9 EGX PRO</strong><div class="pts">${c.score.v16}</div><small>${esc(v?.evidenceGrade||'N/A')}</small></div></div>
    ...
    <div class="egx-evidence"><span class="egx-pill">Common dates: ${dates.length}</span><span class="egx-pill">${esc(dates.join(' · ')||'N/A')}</span><span class="egx-pill">EGX ONE cost: ${fmt(c.currentCostPct,'%')}</span><span class="egx-pill">V16.9 cost: ${fmt(c.v16CostPct,'%')}</span><span class="egx-pill">Forward: ${esc(c.forwardStatus)} (${c.forwardResolutions})</span></div>
    <div class="egx-warn">${esc(c.caveat)}. Net-return metrics لا تمنح نقاطًا عندما تختلف التكلفة، وFailure لا تُحسب كنقطة ثانية لأنها الوجه العكسي لنفس outcome family.</div>
...
async function init(){
  ...
  try{
    ...
    const host=document.getElementById('compareArea')||document.querySelector('#compare .grid')||document.getElementById('compare');
    render(host,c);return c;
