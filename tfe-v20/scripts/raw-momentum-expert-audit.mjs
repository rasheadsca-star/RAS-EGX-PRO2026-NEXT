...
import { buildRawMomentumSnapshot } from '../src/rawMomentumExpert.js';
...
const outPath = path.join(repoRoot, 'tfe-v20/reports/raw-momentum-expert-audit.json');
...
function evaluateV16(member, raw, signalDate) {
  ...
  if (win.length < HOLD) return { status:'INSUFFICIENT_FUTURE', filled:false, netReturnPct:null };
  const levels = {
    entryLow:Number(member.entryLow), entryHigh:Number(member.entryHigh), stopLoss:Number(member.stopLoss), target1:Number(member.target1)
  };
...
const validationDates=allDates.slice(split);
const snapshots=new Map();
const standaloneRows=[];
const joinedRows=[];

for (const session of sessions) {
  const snapshot=buildRawMomentumSnapshot(documents,session.signalDate);
  snapshots.set(session.signalDate,snapshot);
...
const validation=confirmationReport(validationDates);
const standaloneDevelopment=summarize(subset(standaloneRows,developmentDates),developmentDates);
const standaloneValidation=summarize(subset(standaloneRows,validationDates),validationDates);

const folds=[];
for(let i=0;i<3;i++){
  const start=Math.floor(i*allDates.length/3),end=Math.floor((i+1)*allDates.length/3),dates=allDates.slice(start,end),r=confirmationReport(dates),d=r.deltaFilteredMinusBaseline;
...
const d=validation.deltaFilteredMinusBaseline;
const checks={
  validationSessionsAtLeast12:validationDates.length>=12,
  standaloneValidationPositiveAverage:standaloneValidation.avgBasketNetPct>0,
  standaloneValidationProfitFactorAtLeast110:standaloneValidation.profitFactor>=1.10,
  confirmedValidationFilledAtLeast12:validation.filtered.filled>=12,
  confirmationAverageNetImprovesAtLeast025pp:d.averageNetPct>=0.25,
  confirmationStopNotWorse:d.stopHitPct<=0,
  confirmationProfitFactorNotWorse:validation.filtered.profitFactor>=validation.baseline.profitFactor,
  confirmationDrawdownNotWorse:validation.filtered.maxDrawdownPct>=validation.baseline.maxDrawdownPct,
  positiveDirectionAtLeast2Of3Folds:folds.filter(x=>x.positiveDirection).length>=2
};
const passesRetrospectiveGate=Object.values(checks).every(Boolean);

const report={
  schemaVersion:'raw-cross-sectional-momentum-v1-audit',generatedAt:new Date().toISOString(),
  evidenceClass:'PREREGISTERED_RETROSPECTIVE_POINT_IN_TIME_RESEARCH',
  ...
  frozenPolicy:{
    minimumHistory:80,momentum20:'close/close[-21]-1',momentum60:'close/close[-61]-1',trend50:'close/EMA50-1',liquidity:'median(adjustedClose*volume,20)',
    eligibility:'mom20>0 AND mom60>0 AND trend50>0 AND >=15 nonzero-volume days of 20',
    score:'equal-weight cross-sectional percentile of mom20,mom60,trend50,liquidity',confirmationScoreAtLeast:CONFIRM_SCORE,
    standaloneSelection:'top3',standaloneExecution:'next-session open to third-session close',metaComparisonExecution:'exact legacy entry zone/stop/target; next-session zone touch; three sessions; STOP_FIRST',roundTripCostPct:COST_PCT,
    policyTunedOnOutcomes:false
  },
  split:{method:'oldest two-thirds development, newest one-third validation fixed before first expert result',developmentFrom:developmentDates[0]||null,developmentTo:developmentDates.at(-1)||null,validationFrom:validationDates[0]||null,validationTo:validationDates.at(-1)||null},
  universe:{documents:documents.length},
  standalone:{development:standaloneDevelopment,validation:standaloneValidation},
  confirmation:{development,validation,folds},
  preregisteredChecks:checks,passesRetrospectiveGate,
  disposition:passesRetrospectiveGate?'CANDIDATE_FOR_FRESH_FORWARD_SHADOW':'REJECT_ALPHA_WEIGHT_ZERO',
  promotion:{eligible:false,reason:passesRetrospectiveGate?'Retrospective point-in-time gate passed; fresh forward independent evidence is still mandatory before promotion.':'Preregistered retrospective gate failed; do not tune this V1 policy on observed outcomes.'}
};
