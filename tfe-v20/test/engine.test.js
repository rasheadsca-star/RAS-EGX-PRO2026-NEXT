...
test('unready history cannot fall back into ranking universe and remains explicitly visible',()=>{const snapshot={publishedCandidates:[{ticker:'FALL',rank:1}]};const summary={latestMarketSession:'2026-08-28',symbols:[{ticker:'FALL',symbolVerified:false,availableSessions:120,staleData:false,updateFailed:false,lastSession:'2026-08-28'}]};const s=selectUniverseCandidates(summary,snapshot);assert.equal(s.candidates.length,0);assert.equal(s.discoveryOnlyCandidates.length,1);assert.equal(s.readinessExcludedCandidates.length,1);assert.ok(s.readinessExcludedCandidates[0].reasonCodes.includes('SYMBOL_IDENTITY_NOT_VERIFIED'));assert.equal(s.readinessGate.fallbackMayEnterRanking,false);assert.equal(s.universeMode,'DATA_READINESS_BLOCKED_NO_CURRENT_HISTORY')});
