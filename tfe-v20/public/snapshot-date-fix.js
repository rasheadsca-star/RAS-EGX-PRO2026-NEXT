...
function currentMeta(){
  ...
  return {
    ...
    returned:Number(scan?.summary?.returned??scan?.recommendations?.length??0),
...
function renderDashboardGuard(){
  ...
  const stale=Boolean(meta.currentDate&&frozenDate&&frozenDate<meta.currentDate);
  const noToday=meta.currentDate&&meta.published===0;
  ...
  guard.innerHTML=todayLine+frozenLine+(noToday?'<br><span>عدم وجود توصيات اليوم نتيجة بوابات RC2 الحالية، وليس بسبب توقف تحديث البيانات.</span>':'');
}
...
function renderMonitorGuard(){
  ...
  if(head) head.textContent=stale?'متابعة آخر Snapshot توصيات مجمّد':'متابعة الجلسة للمرشحين';
  if(intro&&stale) intro.innerHTML=`البطاقات أدناه تتابع إشارات Snapshot <b>${esc(fmtDate(signalDate))}</b> فقط. جلسة السوق الحالية هي <b>${esc(fmtDate(meta.currentDate))}</b>، ولا يتم تقديم هذه الإشارات القديمة كتوصيات اليوم.`;
  ...
  if(stale){
    guard.hidden=false;
    guard.innerHTML=`<b>جلسة السوق الحالية: ${esc(fmtDate(meta.currentDate))}</b> · ${esc(meta.scanned)} سهم تم فحصه · ${meta.published>0?`${esc(meta.published)} توصية منشورة`:'لا توجد توصيات منشورة اليوم'}.<br><b>Snapshot المتابع أدناه: ${esc(fmtDate(signalDate))}</b> — سجل تاريخي/Forward Evidence فقط.`;
    panel.querySelectorAll('.sm-kpi span').forEach(el=>{
      if(el.textContent.includes('من توصيات الجلسة')) el.textContent=`من توصيات Snapshot ${fmtDate(signalDate)}`;
    });
  }else{
    guard.hidden=true;
  }
...
window.addEventListener('rc2:ui-scan',()=>queueMicrotask(apply));
window.addEventListener('rc2:session-monitor',()=>queueMicrotask(apply));
window.addEventListener('storage',e=>{if(e.key===ARCHIVE_KEY)queueMicrotask(apply)});
