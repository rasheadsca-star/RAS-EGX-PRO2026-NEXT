...
export function backtestHistory({ ticker, rows, historyMeta = {}, minBars = POLICY.minBars }) {
  const bars = normalizeBars(rows).bars;
  const trades = [], expired = [];
  let i = minBars - 1;
  while (i < bars.length - 1) {
    ...
    trades.push({
      ticker, signalDate: bars[i].date, entryDate: bars[entry.j].date, exitDate: bars[exit.j].date,
      outcome: exit.outcome, netPct: round((exit.price - entry.price) / entry.price * 100 - POLICY.roundTripCostPct, 2),
      signalResearchScore: a.scores.research, structuralNetRR: a.tradePlan.structuralNetRR,
    });
    i = exit.j + 1;
  }
...
export function summarizeBacktest(trades, expired = []) {
  ...
  return { trades, expired, summary: {
    entered: n, target1Pct: round(t1/n*100,1), stopPct: round(stop/n*100,1), positivePct: round(wins.length/n*100,1),
    avgNetPct: round(avg(trades.map((x) => x.netPct)),2), profitFactor: gl ? round(gp/gl,2) : gp > 0 ? 'INF' : null,
