...
const runtime=path.join(__dirname,'.backtest-consensus-v16-quality-gate-v2.runtime.cjs');
let text=fs.readFileSync(source,'utf8');

function replaceOnce(from,to,label){
  const n=text.split(from).length-1;
  if(n!==1)throw new Error(`QUALITY_V2_REWRITE_${label}_EXPECTED_ONCE_GOT_${n}`);
...
text=text.replaceAll('consensus-pipeline-v1-backtest','consensus-v16-quality-gate-v2-backtest');
text=text.replace("schemaVersion:'egx-consensus-pipeline-v1-backtest'", "schemaVersion:'egx-consensus-v16-quality-gate-v2-backtest'");
text=text.replace('# EGX Consensus Pipeline V1 — Uncapped Walk-Forward','# EGX Consensus — V16 Quality Gate V2 — 60-Session Validation');
...
text=text.replace("limitations:['V16 60-session list", "limitations:['V16 Quality Gate V2 requires historical Top-10 lift > 1 and a 3-of-4 same-session relative-quality vote; no realized-return threshold and no fixed Top-N cap.','The 11-session V16 warm-up exists only to leave exactly 60 fully evaluable sessions after future-window eligibility.','V16 60-session list");
