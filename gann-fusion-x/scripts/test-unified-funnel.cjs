...
console.log(JSON.stringify({ok:!process.exitCode,session:current.sessionDate,analyzed:all.length,funnel:spec,daily:dailyRows.map(x=>({ticker:x.ticker,rankScore:x.decisionFunnel.speculative.rankScore,executionQuality:x.decisionFunnel.speculative.executionQuality,tier:x.decisionFunnel.speculative.executionTier})),forwardGann:gann,weekly:(current.weeklyTop||[]).map(x=>x.ticker),watchTop:(current.watchRadar||[]).slice(0,8).map(x=>({ticker:x.ticker,reason:x.decisionFunnel.speculative.reasonCode,rankScore:x.decisionFunnel.speculative.rankScore})),rejectedTop:(current.rejectedRadar||[]).slice(0,5).map(x=>({ticker:x.ticker,reason:x.decisionFunnel.speculative.reasonCode}))},null,2));
