...
const candidates=(index.stocks||[]).filter(x=>x.historyAvailable).sort((a,b)=>Number(b.inTodayRecommendations)-Number(a.inTodayRecommendations)||(Number(a.technicalRank)||9999)-(Number(b.technicalRank)||9999)).slice(0,60);
...
const counts={speculative:0,medium:0,long:0},plans={speculative:[],medium:[],long:[]};for(const a of analyses){for(const h of Object.keys(counts)){const p=Planner.buildPlan(a,h,{portfolioValue:100000,riskPct:h==='speculative'?.5:h==='medium'?.65:.75,verifiedFundamentals:Boolean(a.sepaEvidence?.fundamentals?.verified)});plans[h].push(p);if(p.eligible)counts[h]++;assert(p.levels.stopLoss<p.levels.referenceEntry);assert(p.levels.target1>p.levels.referenceEntry);assert(p.levels.target2>p.levels.target1);assert(p.levels.target3>p.levels.target2)}}
for(const h of Object.keys(plans)){const top=plans[h].sort((a,b)=>b.score-a.score).slice(0,8).map(p=>({ticker:p.ticker,eligible:p.eligible,hScore:p.score,fusion:p.analysis.score,class:p.analysis.classification?.code,fresh:p.analysis.sessionDate===index.marketDate,trend:p.analysis.parts?.trend?.score,fund:p.analysis.parts?.fundamentals?.score,breakout:{near:p.analysis.parts?.breakout?.near,confirmed:p.analysis.parts?.breakout?.confirmed},gann:p.analysis.parts?.gannTime?.active,riskPct:p.levels.riskPct,liq:p.analysis.marketMeta?.liquidityPercentile,reason:p.reasonAr}));console.log('DIAGNOSTIC',h,JSON.stringify(top))}
