...
for(const r of currentGann)if(r.forwardEligibilitySchema!=='v2-ready-only'||r.dataReadiness?.status!=='READY'||r.action!=='ACTIONABLE'||!(finite(r.portfolioPct)&&Number(r.portfolioPct)>0))add('critical','CURRENT_GANN_CANDIDATE_NOT_STRICT_READY','Current GANN candidate violates READY-only capture.',{ticker:r.ticker});
...
const out={schemaVersion:'forward-shadow-destroyer-v1',generatedAt:new Date().toISOString(),passed:critical===0&&major===0,critical,major,minor,findings,summary:{marketSession:report.marketSession||null,status:validation.status||null,eligibleGannSignals:Number(validation.eligibleSignals||0),validEvaluatedOutcomes:Number(validation.validEvaluatedOutcomes||0),evaluatedForwardSessions:evaluatedSessions,minimumForwardSessions:minimum,legacyGannSignalsExcluded:legacyGann.length,currentReadyCandidates:currentGann.length},policy:{readyOnlyGannCapture:true,pointInTimeReadinessRequired:true,missingNeverZero:true,unfilledReturnUnknown:true,entryStrictlyAfterSignal:true,legacyEvidenceExcludedNotRewritten:true,noAutomaticPromotion:true,noPerformanceClaimBeforeMinimumForwardSample:true}};
