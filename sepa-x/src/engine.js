...
import { structuralPrecisionGeometryEngine } from './precision-geometry.js';
...
export async function scanMarket({provider=new MarketDataProvider(DEFAULT_CONFIG),config=DEFAULT_CONFIG,limit=null,previousScan=null}={}){
  ...
  const loaded=await workerMap(selected,config.cache.concurrency,async(entry)=>{
    ...
    const structureRetest=structureRetestEngine(bars,config),historicalCycle=historicalCycleEngine(bars,config),precisionGeometry=structuralPrecisionGeometryEngine(bars,config);
    const sector=entry.sector||entry.industry||entry.fundamentals?.sectorModel||'UNKNOWN';
    return {symbol:entry.ticker,name:entry.companyNameEn||entry.companyNameAr||entry.ticker,nameAr:entry.companyNameAr||null,nameEn:entry.companyNameEn||null,bars,entryMeta:entry,data,liquidity,trend,rs,fundamentals,catalyst,vcp,volume,tightness,pivot,structureRetest,historicalCycle,precisionGeometry,sector,historyMeta:stock.meta,price_data_as_of:stock.meta.priceDataAsOf,fundamentals_as_of:stock.meta.fundamentalsAsOf,analysis_generated_at:generatedAt};
  });
  ...
  const coverage={
    ...
    BreakoutRetestConfirmed:finalRows.filter(x=>x.structureRetest?.raw?.resistance?.pass).length,SupportReclaimConfirmed:finalRows.filter(x=>x.structureRetest?.raw?.support?.pass).length,HistoricalCycleAligned:finalRows.filter(x=>x.historicalCycle?.pass).length,StructuralPrecisionReady:finalRows.filter(x=>x.precisionGeometry?.pass).length,
  ...
  const compact=(x)=>({
    ...
    audit_stages:{data_integrity:x.data,liquidity:x.liquidity,trend:x.trend,relative_strength:x.rs,fundamentals:x.fundamentals,catalyst:x.catalyst,vcp:x.vcp,volume:x.volume,tightness:x.tightness,pivot:x.pivot,entry:x.entry,risk:x.risk,structure_retest:x.structureRetest,historical_cycle:x.historicalCycle,precision_geometry:x.precisionGeometry},
  ...
  return serializable({
    ...
    strategy_lab:{mode:config.strategies?.challengerMode===false?'ACTIVE':'CHALLENGER',eligibilityImpact:config.strategies?.challengerMode===false?'CONFIGURED_ACTIVE':'NONE_CHALLENGER_MODE',engines:['BREAKOUT_RETEST','SUPPORT_RECLAIM','HISTORICAL_CYCLE','STRUCTURAL_PRECISION'],calibrationRequiredBeforePromotion:true},
