...
export async function scanMarket({provider=new MarketDataProvider(DEFAULT_CONFIG),config=DEFAULT_CONFIG,limit=null,previousScan=null}={}){
  ...
  const loaded=await workerMap(selected,config.cache.concurrency,async(entry)=>{
    ...
    return {symbol:entry.ticker,name:entry.companyNameEn||entry.companyNameAr||entry.ticker,nameAr:entry.companyNameAr||null,nameEn:entry.companyNameEn||null,bars,entryMeta:entry,data,liquidity,trend,rs,fundamentals,catalyst,vcp,volume,tightness,pivot,sector,historyMeta:stock.meta,price_data_as_of:stock.meta.priceDataAsOf,fundamentals_as_of:stock.meta.fundamentalsAsOf,analysis_generated_at:generatedAt};
  });
  ...
  const r252Ready=finalRows.filter(x=>finite(x.rs.returns.R252)).length;
  const week52Ready=finalRows.filter(x=>finite(x.trend.raw.high52w)&&finite(x.trend.raw.low52w)).length;
  const completeLongHistory=finalRows.filter(x=>x.bars.length>=config.market.requiredHistorySessions&&finite(x.trend.raw.SMA200)&&finite(x.rs.returns.R252)&&finite(x.trend.raw.high52w)&&finite(x.trend.raw.low52w)).length;
  const coverage={
  ...
  const compact=(x)=>({
    ...
    history_metrics:{session_count:x.data.raw.sessionCount,required_sessions:x.data.raw.requiredSessionCount,source:x.data.raw.longHistorySource,range:x.data.raw.longHistoryRange,coverage_start:x.data.raw.longHistoryCoverageStart,coverage_end:x.data.raw.longHistoryCoverageEnd,SMA200:x.trend.raw.SMA200,SMA200_20_ago:x.trend.raw.SMA200_20_ago,R252:x.rs.returns.R252,high52w:x.trend.raw.high52w,low52w:x.trend.raw.low52w,distance_from_52w_high_pct:x.trend.raw.distance_from_52w_high_pct,distance_above_52w_low_pct:x.trend.raw.distance_above_52w_low_pct,complete:x.bars.length>=config.market.requiredHistorySessions&&finite(x.trend.raw.SMA200)&&finite(x.rs.returns.R252)&&finite(x.trend.raw.high52w)&&finite(x.trend.raw.low52w)},
