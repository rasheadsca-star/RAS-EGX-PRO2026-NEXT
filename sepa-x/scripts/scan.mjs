...
console.log(JSON.stringify({engineId:scan.engineId,generatedAt:scan.generatedAt,market:scan.market_status,coverage:scan.market_coverage,concentration:scan.concentration_policy,top:scan.top_recommendations.map(x=>({rank:x.conviction_rank,symbol:x.symbol,score:x.final_score,conviction:x.concentration_score,status:x.status,target:x.target_plan?.primaryTarget?.price,rr:x.reward_risk})),noHighConviction:scan.no_high_conviction_setup},null,2));
