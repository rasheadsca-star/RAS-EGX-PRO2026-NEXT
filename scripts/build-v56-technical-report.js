...
function avg(a) { const xs = a.filter(Number.isFinite); return xs.length ? xs.reduce((s,x)=>s+x,0)/xs.length : null; }
function std(a) { const m = avg(a); if (m === null) return null; const xs = a.filter(Number.isFinite); return Math.sqrt(xs.reduce((s,x)=>s+(x-m)*(x-m),0)/Math.max(1,xs.length-1)); }
...
const referenceSessionDate = truth.selectedSessionDate || history.referenceSessionDate || history.summary?.latestSessionDate || null;

const sectors = new Map();
for (const r of [].concat(sectorReport.sectors || [], sectorReport.sectorRows || [], sectorReport.ranking || [])) { if (r && r.sector) sectors.set(r.sector, r); }

const symbols = [];
let rejectedNonTradingPoints = 0;
for (const [symbol, rawPoints] of Object.entries(history.symbols || {})) {
  const dedupe = new Map();
  for (const p of Array.isArray(rawPoints) ? rawPoints : []) {
    const date=String(p?.date||'');
    if (!regularEgxDate(date) || (referenceSessionDate && date > referenceSessionDate) || num(p?.close) === null) { rejectedNonTradingPoints += 1; continue; }
    dedupe.set(date, p);
  }
  const points = [...dedupe.values()].sort((a,b)=>String(a.date).localeCompare(String(b.date)));
  ...
  symbols.push({ symbol, points: closes.length, firstDate: points[0]?.date, lastDate: points[points.length - 1]?.date, lastPrice: last, dailyChangePct: pctChange(prev, last), change50Pct: pctChange(closes[0], last), sma20, sma50, support, resistance, volatilityPct: volPct, avgVolume20, avgTurnover20, trendScore, liquidityScore, riskScore, confidence, signal });
}

symbols.sort((a,b) => (b.confidence || 0) - (a.confidence || 0));
const report = {
  version: '5.6.1-v17-session-truth', generatedAt: new Date().toISOString(), referenceSessionDate,
  policy: { verifiedSessionOnly: true, fridaySaturdayExcluded: true, datesAfterVerifiedSessionExcluded: true, duplicateSessionDatesCollapsed: true },
  sourceHealth: { scanMode: sourceHealth.scanMode, lastSuccessAt: sourceHealth.lastSuccessAt, totalUniverse: sourceHealth.totalUniverse, cacheRows: sourceHealth.cacheRows },
  historyStatus: history.status || {},
  summary: { symbols: symbols.length, withAtLeast20Sessions: symbols.filter(s => s.points >= 20).length, with50Sessions: symbols.filter(s => s.points >= 50).length, topConfidence: symbols[0]?.symbol || null, rejectedNonTradingPoints },
  symbols,
...
console.log(`technical-50-report generated for verified session ${referenceSessionDate}: ${symbols.length} symbols; rejected ${rejectedNonTradingPoints} invalid/non-trading points`);
