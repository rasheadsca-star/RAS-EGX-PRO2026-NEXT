...
const maxPublished=Number(policy.fullMarketNativeSelection?.maximumPublishedResearchCandidates||30),declaredPublished=Number(selection.summary?.publishedResearchCandidateCount??0);
check((native.publishedCandidates||[]).length===declaredPublished&&(native.publishedCandidates||[]).length<=maxPublished,'NATIVE_CURRENT_PUBLISHED_COUNT');
...
const eligible=selection.eligibleResearchRanking||[];check(eligible.length===Number(selection.summary?.nativeResearchRecommendationCount||0),'NATIVE_CURRENT_ELIGIBLE_COUNT');check(pub.length===Math.min(eligible.length,maxPublished),'NATIVE_CURRENT_PUBLICATION_CUTOFF');check(pub.every((r,i)=>r.ticker===eligible[i]?.ticker),'NATIVE_CURRENT_NOT_PREFIX');
...
const report={schemaVersion:'20.0.0-native-current-regression-1',generatedAt:new Date().toISOString(),ok:failures.length===0,failedCount:failures.length,failures,evidence:{sessionDate:native.sessionDate,engineId:native.engineId,publishedCount:pub.length,eligibleCount:eligible.length,maximumPublishedCount:maxPublished,rankingDigest:native.rankingDigest,freezeId:freeze.freezeId,compositeModelDigest:freeze.compositeModelDigest},checks:{nativeArtifactOwnsRanking:true,regressionOwnsRanking:false,fullMarketIndependent:true,legacyContributionZero:true,publicationCountRespectsMaximum:true,exactScoreTieBreakNonMutating:true,v1Frozen:true,executionSeparated:true}};
