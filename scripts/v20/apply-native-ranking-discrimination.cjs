...
results.push(patch(
  ...
  'V20_SAFETY_STRENGTH_TIE_BREAK_HELPERS_V2',
  ...
  `const rrScore=v=>pw(v,[[0,0],[.25,20],[.5,35],[1,55],[1.5,70],[2,82],[3,95],[4,100]]),tier=(s,t)=>n(s)===null?'UNRATED':s>=t.RESEARCH_A?'RESEARCH_A':s>=t.RESEARCH_B?'RESEARCH_B':s>=t.RESEARCH_C?'RESEARCH_C':'RESEARCH_D';\n// V20_SAFETY_STRENGTH_TIE_BREAK_HELPERS_V2\nfunction rankingTieBreakMeta(r,p){const tp=r.evidence?.researchTradePlan||{},state=tp.alignment?.state||'UNKNOWN',priorities=p.rankingDiscrimination?.alignmentPriority||{IN_ENTRY_RANGE:0,BELOW_ENTRY_RANGE_WAITING:1,ABOVE_ENTRY_RANGE_DO_NOT_CHASE:2,UNKNOWN:3},price=n(r.price),lo=n(tp.entryLow),hi=n(tp.entryHigh);let distance=null;if(state==='IN_ENTRY_RANGE')distance=0;else if(state==='ABOVE_ENTRY_RANGE_DO_NOT_CHASE')distance=n(tp.alignment?.distanceAboveEntryHighPct)??(price!==null&&hi>0&&price>hi?(price-hi)/hi*100:null);else if(state==='BELOW_ENTRY_RANGE_WAITING')distance=price!==null&&lo>0&&price<lo?(lo-price)/lo*100:null;return{contract:p.rankingDiscrimination?.contract||'V20_SAFETY_STRENGTH_LEXICOGRAPHIC_TIE_BREAK_V2',appliesOnlyOnExactScoreTie:true,mutatesNativeResearchScore:false,canOverrideHigherNativeResearchScore:false,alignmentState:state,alignmentSafetyPriority:Number(priorities[state]??priorities.UNKNOWN??3),scoreBeforeRegimeAndCaps:n(r.nativeResearch?.scoreBeforeRegimeAndCaps),entryDistancePct:round(distance,3),netRiskReward:n(tp.netRiskReward),liquidity2Score:n(r.evidence?.liquidity?.score),srConfluenceScore:n(r.evidence?.supportResistance?.score),technicalScore:n(r.evidence?.technical?.score),discoveryScore:n(r.discovery?.score)}}\nfunction compareEligible(a,b,p){let d=(n(b.nativeResearch?.score)??-1)-(n(a.nativeResearch?.score)??-1);if(d)return d;const x=rankingTieBreakMeta(a,p),y=rankingTieBreakMeta(b,p);d=x.alignmentSafetyPriority-y.alignmentSafetyPriority;if(d)return d;d=(n(y.scoreBeforeRegimeAndCaps)??-Infinity)-(n(x.scoreBeforeRegimeAndCaps)??-Infinity);if(d)return d;d=(n(x.entryDistancePct)??Infinity)-(n(y.entryDistancePct)??Infinity);if(d)return d;for(const k of ['netRiskReward','liquidity2Score','srConfluenceScore','technicalScore','discoveryScore']){d=(n(y[k])??-Infinity)-(n(x[k])??-Infinity);if(d)return d}return a.ticker.localeCompare(b.ticker)};`
));
results.push(patch(
  'scripts/v20/build-full-market-native-selection.cjs',
  'V20_SAFETY_STRENGTH_TIE_BREAK_SORT_V2',
  ...
  `.sort((a,b)=>compareEligible(a,b,p)),/* V20_SAFETY_STRENGTH_TIE_BREAK_SORT_V2 */ranking=eligible.map`
));
results.push(patch(
  'scripts/v20/build-full-market-native-selection.cjs',
  'V20_SAFETY_STRENGTH_TIE_BREAK_FIELDS_V2',
  `alignmentState:r.evidence.researchTradePlan.alignment?.state||null,wasInLegacySeedUniverse:r.wasInLegacySeedUniverse`,
  `alignmentState:r.evidence.researchTradePlan.alignment?.state||null,entryDistancePct:rankingTieBreakMeta(r,p).entryDistancePct,rankingTieBreaker:rankingTieBreakMeta(r,p),/* V20_SAFETY_STRENGTH_TIE_BREAK_FIELDS_V2 */wasInLegacySeedUniverse:r.wasInLegacySeedUniverse`
));
results.push(patch(
  'scripts/v20/build-full-market-native-selection.cjs',
  'V20_ELIGIBLE_FULL_RANKING_PERSISTED_V2',
  ...
  `top5:ranking.slice(0,5),top10:ranking.slice(0,10),eligibleResearchRanking:ranking,/* V20_ELIGIBLE_FULL_RANKING_PERSISTED_V2 */recommendationRanking:ranking.slice(0,p.maximumPublishedResearchCandidates),discoveryRanking`
));
results.push(patch(
  'scripts/v20/build-full-market-native-selection.cjs',
  'V20_RANKING_DISCRIMINATION_CONTRACT_EXPOSED_V2',
  `missingComponentPolicy:'NO_FINAL_RECOMMENDATION_UNLESS_ALL_NON_LEGACY_COMPONENTS_AVAILABLE'},summary:`,
  `missingComponentPolicy:'NO_FINAL_RECOMMENDATION_UNLESS_ALL_NON_LEGACY_COMPONENTS_AVAILABLE',rankingDiscrimination:{...p.rankingDiscrimination,scoreMutation:false,executionInfluence:false}},/* V20_RANKING_DISCRIMINATION_CONTRACT_EXPOSED_V2 */summary:`
));
results.push(patch(
...
results.push(patch(
  ...
  `conservativeCostAwareNetRrRequired:true,minimumNetRiskRewardPreserved:true,safetyStrengthTieBreakPreserved:true,tieBreakUsesPreCapStrength:true,tieBreakCannotMutateScore:true,tieBreakCannotOverrideHigherScore:true,/* V20_RANKING_DISCRIMINATION_REPORT_V2 */v17ExecutionAuthorityPreserved:true`
));
results.push(patch(
  'scripts/v20/build-native-explainability.cjs',
  'V20_EXPLAINABILITY_SELECTION_RANK_MAP_V2',
  ...
  `const legacySeed=new Set((profiles.profiles||[]).map(x=>sym(x.ticker))),legacyRank=new Map((profiles.researchDecisionRanking||[]).map(x=>[sym(x.ticker),n(x.rank)])),selectionRankByTicker=new Map((selection.eligibleResearchRanking||[]).map(x=>[sym(x.ticker),x]));/* V20_EXPLAINABILITY_SELECTION_RANK_MAP_V2 */`
));
results.push(patch(
  'scripts/v20/build-native-explainability.cjs',
  'V20_EXPLAINABILITY_TIE_BREAK_FIELDS_V2',
  `alignmentState:tp.alignment?.state||null,tradePlan:`,
  `alignmentState:tp.alignment?.state||null,entryDistancePct:n(selectionRankByTicker.get(ticker)?.entryDistancePct),rankingTieBreaker:selectionRankByTicker.get(ticker)?.rankingTieBreaker||null,/* V20_EXPLAINABILITY_TIE_BREAK_FIELDS_V2 */tradePlan:`
));
results.push(patch(
  'scripts/v20/build-native-explainability.cjs',
  'V20_EXPLAINABILITY_EXACT_SELECTION_ORDER_V2',
  ...
  `const eligible=[...rows].filter(r=>r.recommendationEligible).sort((a,b)=>(n(selectionRankByTicker.get(a.ticker)?.rank)??9999)-(n(selectionRankByTicker.get(b.ticker)?.rank)??9999));/* V20_EXPLAINABILITY_EXACT_SELECTION_ORDER_V2 */if(eligible.length!==selectionRankByTicker.size||eligible.some((r,i)=>n(selectionRankByTicker.get(r.ticker)?.rank)!==i+1))throw new Error('Explainability ranking order mismatch');`
));
results.push(patch(
  'scripts/v20/build-native-explainability.cjs',
  'V20_EXPLAINABILITY_SELECTION_RANK_ASSIGNED_V2',
  `eligible.forEach((r,i)=>{r.eligibleRank=i+1;`,
  `eligible.forEach((r,i)=>{r.eligibleRank=n(selectionRankByTicker.get(r.ticker)?.rank)??i+1;/* V20_EXPLAINABILITY_SELECTION_RANK_ASSIGNED_V2 */`
));
console.log(JSON.stringify({schemaVersion:'20.0.0-native-ranking-discrimination-patch-2',contract:'V20_SAFETY_STRENGTH_LEXICOGRAPHIC_TIE_BREAK_V2',researchOnly:true,scoreMutation:false,executionInfluence:false,results},null,2));
