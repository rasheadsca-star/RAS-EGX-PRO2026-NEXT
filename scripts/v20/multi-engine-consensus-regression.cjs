...
const non=x.engineRegistry?.monitoredNonVotes||[];check(non.some(e=>e.id==='V17_PRODUCTION_VALIDATION_AUTHORITY'&&e.voteEligible===false),'V17 must be monitored but never double-counted');check(non.some(e=>e.id==='V21'&&e.status==='NOT_PRESENT'),'V21 absence must be explicit');
...
const result={schemaVersion:'20.0.0-multi-engine-consensus-regression-1',generatedAt:new Date().toISOString(),ok:failures.length===0,failedCount:failures.length,failures,sessionDate:x.sessionDate,mainAppAnnotations:(x.current?.mainAppAnnotations||[]).map(a=>({ticker:a.ticker,votes:a.independentVotes,score:a.confirmationScore,label:a.confirmationLabelAr})),governancePreserved:failures.length===0};
