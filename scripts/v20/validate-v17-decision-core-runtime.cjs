...
if(technical.version!=='5.6.1-v17-session-truth')failures.push('V17_TECHNICAL_REPORT_VERSION_DRIFT');
if(rec.sessionDate!==sync.sessionDate||technical.referenceSessionDate!==sync.sessionDate||status.sessionDate!==sync.sessionDate)failures.push('V17_RECOMMENDATION_TECHNICAL_SESSION_MISMATCH');
...
if(technical.policy?.verifiedSessionOnly!==true||technical.policy?.datesAfterVerifiedSessionExcluded!==true||technical.policy?.duplicateSessionDatesCollapsed!==true)failures.push('V17_TECHNICAL_SESSION_TRUTH_POLICY_DRIFT');
