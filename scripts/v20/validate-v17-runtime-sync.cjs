...
const required=[
 'data/market.json','data/final-opportunity-ranking.json','data/history.json','data/recommendations.json','data/technical-50-report.json',
...
check(rows(recommendations.all||recommendations).length>0,'V17_RUNTIME_RECOMMENDATIONS_EMPTY');
check((technical.symbols||[]).length>0,'V17_RUNTIME_TECHNICAL_EMPTY');
