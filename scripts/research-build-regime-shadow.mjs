...
const uiPath=path.join(ROOT,'data','research','ui','latest.json'),publicationPath=path.join(ROOT,'data','research','published','latest.json'),regimeDir=path.join(ROOT,'data','research','regime'),contextDir=path.join(ROOT,'data','research','context'),ledgerDir=path.join(ROOT,'data','research','shadow-ledger'),ledgerPath=path.join(ledgerDir,'latest.json');
...
const context=buildResearchDecisionContext({publication,regime});if(!verifyResearchDecisionContext(context,{publication,regime}))throw new Error('DECISION_CONTEXT_VERIFY_FAILED');fs.mkdirSync(contextDir,{recursive:true});fs.writeFileSync(path.join(contextDir,'latest.json'),JSON.stringify(context,null,2)+'\n');
...
console.log(JSON.stringify({session:regime.session,regime:regime.regime,coverage:regime.coverage,breadth:regime.breadth,policy:regime.policy,regimeHash:regime.regimeHash,decisionContextHash:context.contextHash,currentRecommendationContexts:context.recommendations.length,shadowStartAfterSession:ledger.startAfterSession,shadowEntries:ledger.entries.length,shadowLedgerHash:ledger.ledgerHash},null,2));
