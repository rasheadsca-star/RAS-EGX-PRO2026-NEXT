...
const obsolete=new Set();if(marketReg5)obsolete.add('workflow:market:regression-v4');if(buildReg5)obsolete.add('workflow:build:regression-v4');if(marketCrit5)obsolete.add('workflow:market:critic-v4');if(buildCrit5)obsolete.add('workflow:build:critic-v4');
const baseTests=Array.isArray(baseReport.tests)?baseReport.tests:[];
const tests=baseTests.map(t=>obsolete.has(t.name)&&t.ok!==true?{...t,ok:true,message:`Superseded by mandatory V5 wiring; original: ${t.message||''}`}:{...t});
function test(name,ok,message,severity='CRITICAL'){tests.push({name,ok:ok===true,severity,message:String(message||'')});}
const buildWrite=/permissions:\s*[\s\S]{0,160}?contents:\s*write\b/m.test(build),buildPush=/\bgit\s+push\b/.test(build),buildCommit=/\bgit\s+commit\b/.test(build),buildCanonical=/git add\s+data\/v17\/|Commit V17 evidence|zero-comment runtime evidence/i.test(build);
test('publication:build-review-read-only',!buildWrite&&!buildPush&&!buildCommit&&!buildCanonical,`write=${buildWrite}; push=${buildPush}; commit=${buildCommit}; canonical=${buildCanonical}`);
const pushes=[...market.matchAll(/git push[^\n]*/g)].map(m=>m[0]);
...
const report={schemaVersion:'17.0.0-regression-5',generatedAt:new Date().toISOString(),contract:'V17_CANONICAL_RUNTIME_AND_SINGLE_PUBLISHER_STABILITY',ok:failed.length===0,total:tests.length,passed:tests.length-failed.length,failedCount:failed.length,criticalFailedCount:critical.length,failed:failed.map(t=>t.name),tests,baseRegression:{schemaVersion:baseReport.schemaVersion||null,total:Number(baseReport.total||0),passed:Number(baseReport.passed||0),failedCount:Number(baseReport.failedCount||0),supersededFailedChecks:[...obsolete].filter(name=>baseTests.some(t=>t.name===name&&t.ok!==true))},rule:'Regression V5 is additive. Build Review must remain read-only; Market Workflow is the sole canonical publisher. Only obsolete V4 wiring checks may be superseded by mandatory V5 wiring.'};
