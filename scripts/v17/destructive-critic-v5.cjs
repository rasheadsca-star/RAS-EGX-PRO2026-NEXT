...
const buildCommitLines=executableGitLines(buildWorkflow,'commit');
...
if(buildPushLines.length)add('CRITICAL','BUILD_REVIEW_CAN_PUSH',buildPushLines.join(' | '),'v17-build-review');
if(buildCommitLines.length||buildHasCanonicalCommitStep)add('MAJOR','BUILD_REVIEW_PUBLISHES_CANONICAL_EVIDENCE',(buildCommitLines.join(' | ')||'canonical evidence commit/stage step found'),'v17-build-review');

const marketPushes=executableGitLines(marketWorkflow,'push');
...
for(const [label,obj] of [['browser',browser],['review',review],['regression',regression]]){const t=time(obj.generatedAt);if(decisionTime&&t&&t+5*60*1000<decisionTime)add('MAJOR','STALE_PUBLISHED_EVIDENCE',`${label} generatedAt=${obj.generatedAt} materially predates decision=${decision.generatedAt}`,`data/v17/${label}`);}
