...
if (require.main === module) {
  ...
  const output = buildNewsDataset({
    universe: market.results,
    events: input.events,
    asOf: new Date(),
    sourceHealth: input.sourceHealth,
    coverageTickers: input.coverageTickers,
    officialCoverageTickers: input.officialCoverageTickers,
    verifiedSecondaryCoverageTickers: input.verifiedSecondaryCoverageTickers,
  });
