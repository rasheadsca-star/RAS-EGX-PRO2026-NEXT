...
const ignoredInheritedFindings=inherited.filter(f=>f?.code==='CRITIC_V2_NOT_IN_WORKFLOW'&&v3MarketMandatory);
const findings=inherited.filter(f=>!(f?.code==='CRITIC_V2_NOT_IN_WORKFLOW'&&v3MarketMandatory));
...
if(!v3MarketMandatory)add('MAJOR','CRITIC_V3_NOT_MANDATORY_MARKET','market workflow does not run Critic V3','workflow');
if(!v3BuildMandatory)add('MAJOR','CRITIC_V3_NOT_MANDATORY_BUILD_REVIEW','build review does not run Critic V3','v17-build-review');
const counts=findings.reduce((a,f)=>(a[f.severity]=(a[f.severity]||0)+1,a),{CRITICAL:0,MAJOR:0,MINOR:0,INFO:0});
const report={schemaVersion:'17.0.0-destructive-critic-3',generatedAt:new Date().toISOString(),critic:'V17_DESTRUCTIVE_ADVERSARIAL_REVIEWER_V3',verdict:findings.length===0?'NO_COMMENTS':'COMMENTS_FOUND',counts,totalFindings:findings.length,findings,baseCritic:{schemaVersion:baseReport.schemaVersion||null,verdict:baseReport.verdict||null,totalFindings:Number(baseReport.totalFindings||0),ignoredSupersededWiringFindings:ignoredInheritedFindings.map(f=>f.code)},coverage:{...(baseReport.coverage||{}),historicalAllocationSemantics:true,finalizerAttestation:true,dualWorkflowCriticEnforcement:true,criticVersionSupersessionHandledExplicitly:true},rule:'Critic V3 is additive. Only the obsolete V2 direct-wiring finding may be superseded when V3 is mandatory; every substantive V2 finding remains blocking.'};
