...
const v4Market=workflow.includes('destructive-critic-v4.cjs'),v4Build=buildReview.includes('destructive-critic-v4.cjs'),reg4Market=workflow.includes('regression-v4.cjs'),reg4Build=buildReview.includes('regression-v4.cjs');
const supersededNames=new Set();
if(v4Market)supersededNames.add('workflow:market:critic-v3');
if(v4Build)supersededNames.add('workflow:build:critic-v3');
if(reg4Market)supersededNames.add('workflow:market:regression-v3');
if(reg4Build)supersededNames.add('workflow:build:regression-v3');
const baseTests=Array.isArray(baseReport.tests)?baseReport.tests:[];
const tests=baseTests.map(t=>supersededNames.has(t.name)&&t.ok!==true?{...t,ok:true,message:`Superseded by mandatory V4 wiring; original check was: ${t.message||''}`}:{...t});
...
test('workflow:market:regression-v4',reg4Market,'market workflow runs regression v4');
test('workflow:build:regression-v4',reg4Build,'build review runs regression v4');
