...
(async()=>{
  const market=read('data/market.json',{rows:[]}),marketRows=rowsOf(market),direct=read('data/mubasher-support-resistance-direct.json',{rows:[]});
  ...
  const nativeMajority=majority(nativeDates),pageMajority=majority(pageDates),directMajority=majority(directDates),priceEvidenceMajority=pageMajority.date?pageMajority:nativeMajority,selectedSessionDate=priceEvidenceMajority.date||null;
  const priceSourceVerified=Boolean(selectedSessionDate&&priceEvidenceMajority.count>=MIN_PRICE_CONFIRMATIONS&&priceEvidenceMajority.count/Math.max(1,priceEvidenceMajority.total)>=0.6),calendarValid=isRegularTradingWeekday(selectedSessionDate),directConflict=Boolean(directMajority.date&&selectedSessionDate&&directMajority.date!==selectedSessionDate),executionSafe=Boolean(priceSourceVerified&&calendarValid);
  const evidence={engine:'V17_PRICE_SOURCE_SESSION_TRUTH',verifiedAt:new Date().toISOString(),selectedSessionDate,priceSourceVerified,calendarValid,directValidationConflict:directConflict,executionSafe,priceEvidence:{source:pageMajority.date?'LIVE_PRICE_PAGE_LAST_UPDATE':nativeMajority.date?'NATIVE_MARKET_ROW_SESSION':'UNAVAILABLE',majority:priceEvidenceMajority,minimumConfirmations:MIN_PRICE_CONFIRMATIONS,samples:pageSamples},directValidation:{role:'VALIDATION_ONLY_NOT_HARD_DEPENDENCY',majority:directMajority,sessionConflict:directConflict,conflictBlocksPriceSessionTruth:false},policy:{workflowRunDateIsNeverTradingSessionEvidence:true,fridaySaturdayNeverAcceptedAsRegularTradingSession:true,executionRequiresVerifiedPriceSourceSession:true,directMubasherSrDoesNotDefinePriceTruth:true,externalValidationMismatchDoesNotInvalidatePriceSessionTruth:true,missingYearInSourceTextIsInferredConservatively:true}};
  if(executionSafe){mutateSession('data/market.json',selectedSessionDate,evidence);mutateSession('data/source-health.json',selectedSessionDate,evidence);mutateSession('data/fetch-status.json',selectedSessionDate,evidence);if(process.env.GITHUB_ENV)fs.appendFileSync(process.env.GITHUB_ENV,`EGX_SESSION_DATE=${selectedSessionDate}\n`,'utf8');}else evidence.blockingReason=!selectedSessionDate?'PRICE_SOURCE_SESSION_UNAVAILABLE':!priceSourceVerified?'PRICE_SOURCE_SESSION_NOT_SUFFICIENTLY_VERIFIED':'SOURCE_DATE_IS_NOT_REGULAR_EGX_TRADING_WEEKDAY';
  write(OUT,evidence);console.log(JSON.stringify({selectedSessionDate,pageMajority,directMajority,priceSourceVerified,calendarValid,directConflict,executionSafe},null,2));
