...
if (current) {
  if (current.schemaVersion !== '17.0.0-rc1') add('MAJOR', 'UNEXPECTED_SCHEMA', `Unexpected schema ${current.schemaVersion}.`, 'data/v17/current.json');
  ...
  for (const row of rows) {
    ...
    if (row.executionRules?.chaseForbidden !== true || row.executionRules?.requireLiquidityConfirmation !== true) {
      add('MAJOR', 'WEAK_EXECUTION_RULES', `${row.ticker} lacks mandatory anti-chase or liquidity confirmation.`, `recommendations.${row.ticker}.executionRules`);
    }
  }
  ...
  const gateActuallyPassed = gate.sampleGatePassed === true && gate.timeGatePassed === true;
  if (current.readiness?.professionalEvidenceReady === true && !gateActuallyPassed) {
    add('CRITICAL', 'FALSE_PROFESSIONAL_CLAIM', 'Professional readiness is true before both live gates pass.', 'readiness.professionalEvidenceReady');
  }
...
if (appFiles.every(exists)) {
  ...
  if (!appSource.includes('قوة السوق') || !appSource.includes('الدليل الحي')) {
    add('MAJOR', 'READINESS_LABELS_UNCLEAR', 'UI must visibly separate market strength from live evidence.', 'preview-v17/app');
  }
...
const report = {
  schemaVersion: '17.0.0-review',
  generatedAt: new Date().toISOString(),
  reviewer: 'V17_INDEPENDENT_CRITIC_GATE',
