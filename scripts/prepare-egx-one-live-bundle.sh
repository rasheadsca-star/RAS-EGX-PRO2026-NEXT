...
mkdir -p \
  ...
  "$OUT/data/research/shadow-ledger" \
...
for f in \
  ...
  data/research/live/latest.json; do
...
console.log(`EGX_ONE_BUNDLE_VERIFIED:SESSION=${s.signalSession}:RECS=${s.recommendations.length}:TRIGGERED=${perf.triggered}:TARGET=${perf.target1HitRatePct}:STOP=${perf.stopRatePct}`);
NODE
