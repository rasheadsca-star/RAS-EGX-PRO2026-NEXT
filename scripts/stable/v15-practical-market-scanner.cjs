...
const MODELS=[
  ...
  {id:'LIQUID_LEADERS',labelAr:'قيادات سائلة قوية نسبيًا',profile:'FAST',eligible:f=>f.trend&&f.ret5>0&&f.ret20>3&&f.rs20>2&&f.vr>=0.75&&f.turnover>=5000000&&f.r14>=50&&f.r14<=78,score:f=>Math.log10(Math.max(f.turnover,1))*4+clamp(f.ret5,-5,20)*1.8+clamp(f.ret20,-10,60)*.35+clamp(f.rs20,-10,50)*1.7+Math.min(f.vr,3)*4+f.rangePos*8},
  {id:'HOT_MOMENTUM',labelAr:'استمرار زخم ساخن',profile:'FAST',eligible:f=>f.ret5>=8&&f.ret20>=12&&f.rs20>=7&&f.vr>=0.8&&f.r14>=76&&f.r14<=94&&f.turnover>=1000000,score:f=>clamp(f.ret5,0,35)*2+clamp(f.ret20,0,80)*.45+clamp(f.rs20,0,70)*1.4+Math.min(f.vr,5)*5+f.rangePos*8-Math.max(0,f.atrPct-10)*2},
  {id:'PRE_BREAKOUT_ACCUMULATION',labelAr:'تجميع قبل الاختراق',profile:'FAST',eligible:f=>f.trend&&f.ret5>=1&&f.ret20>=0&&f.rs20>=-3&&f.vr>=1.2&&f.rangePos>=0.55&&f.breakoutPct>=-7&&f.breakoutPct<=2&&f.r14>=50&&f.r14<=76,score:f=>f.ret5*2+f.ret20*.3+f.rs20+Math.min(f.vr,5)*7+f.rangePos*15-Math.abs(f.breakoutPct)*.8-Math.abs(f.r14-62)*.4},
  {id:'REVERSAL_CONFIRMATION',labelAr:'انعكاس مبكر مؤكد بالحجم',profile:'FAST',eligible:f=>!f.trend&&f.ret1>=1.5&&f.ret3>0&&f.ret5<=8&&f.r14>=28&&f.r14<=58&&f.vr>=1.2&&f.close>f.open&&f.close>f.s10,score:f=>f.ret1*4+f.ret3*2-Math.min(0,f.ret5)*1.2+Math.min(f.vr,5)*7+(58-f.r14)*.5+f.rangePos*6}
];
const PROFILES={
...
function missedOpportunities(store,latestDates){
  const result=[];
  for(const date of latestDates){const list=store.byDate.get(date)||[];for(const f of list){const future=f.rows[f.index+3],next=f.rows[f.index+1];if(!future||!next)continue;const r3=pct(future.close,f.close),gap1=pct(next.open,f.close);if(r3>=5)result.push({signalDate:date,ticker:f.ticker,companyNameAr:f.companyNameAr,forward3Pct:round(r3,2),nextOpenGapPct:round(gap1,2),averageTurnover20Egp:round(f.turnover,0),ret5:round(f.ret5,2),ret20:round(f.ret20,2),relativeStrength20:round(f.rs20,2),volumeRatio20:round(f.vr,2),breakoutPct:round(f.breakoutPct,2),rsi14:round(f.r14,1),trend:f.trend,matchedModels:MODELS.filter(m=>m.eligible(f)).map(m=>m.id)});}}
  const realistic=result.filter(x=>x.averageTurnover20Egp>=1000000&&Math.abs(x.nextOpenGapPct)<=15);
  const missed=result.filter(x=>!x.matchedModels.length).sort((a,b)=>b.forward3Pct-a.forward3Pct),realisticMissed=realistic.filter(x=>!x.matchedModels.length).sort((a,b)=>b.forward3Pct-a.forward3Pct);
  return {rawWinners:result.length,rawCaptured:result.length-missed.length,rawCaptureRatePct:result.length?round((result.length-missed.length)/result.length*100,1):null,tradableWinners:realistic.length,tradableCaptured:realistic.length-realisticMissed.length,tradableCaptureRatePct:realistic.length?round((realistic.length-realisticMissed.length)/realistic.length*100,1):null,topMissed:realisticMissed.slice(0,40),topWinners:realistic.sort((a,b)=>b.forward3Pct-a.forward3Pct).slice(0,40)};
}
...
function main(){
  ...
  const modelResults=MODELS.map(model=>{const dev=evaluateModel(store,model,development),val=evaluateModel(store,model,validation),testResult=evaluateModel(store,model,test);const valEdge=n(val.metrics.targetRatePct,0)-n(val.metrics.stopRatePct,100),testEdge=n(testResult.metrics.targetRatePct,0)-n(testResult.metrics.stopRatePct,100);const robustScore=Math.min(n(val.metrics.averageReturnPct,-99),n(testResult.metrics.averageReturnPct,-99))*10+Math.min(n(val.metrics.profitFactor,0),n(testResult.metrics.profitFactor,0))*4+Math.min(n(val.metrics.winRatePct,0),n(testResult.metrics.winRatePct,0))*.06+Math.min(valEdge,testEdge)*.12;return {id:model.id,labelAr:model.labelAr,profile:model.profile,development:dev.metrics,validation:val.metrics,test:testResult.metrics,validationPassed:passValidation(val.metrics),testPassed:passValidation(testResult.metrics),selectionScore:round(robustScore,2)};}).sort((a,b)=>b.selectionScore-a.selectionScore);
  const validatedModels=modelResults.filter(x=>x.validationPassed&&x.testPassed);
  const selected=validatedModels[0]||null;
  const latestDate=store.dates.at(-1), latestAll=store.byDate.get(latestDate)||[], opportunities=[];
  for(const result of validatedModels){const model=MODELS.find(m=>m.id===result.id),p=PROFILES[model.profile],pool=latestAll.filter(model.eligible).sort((a,b)=>model.score(b)-model.score(a)).slice(0,10);for(const [localRank,f] of pool.entries()){const extended=(model.id!=='HOT_MOMENTUM'&&(f.r14>78||f.ret20>60||f.breakoutPct>12))||(model.id==='HOT_MOMENTUM'&&(f.r14>92||f.ret20>100));const stop=f.close-f.a14*p.stopAtr,target=f.close+f.a14*p.targetAtr;opportunities.push({ticker:f.ticker,companyNameAr:f.companyNameAr,strategyId:model.id,strategyLabelAr:model.labelAr,profile:model.profile,modelRobustScore:result.selectionScore,localRank:localRank+1,combinedScore:result.selectionScore*3+(10-localRank)*2,extended,score:round(model.score(f),2),close:round(f.close,4),entryLow:round(f.close-f.a14*.15,4),entryHigh:round(f.close+f.a14*.15,4),stopLoss:round(stop,4),target1:round(target,4),riskReward:round((target-f.close)/(f.close-stop),2),holdingSessions:p.maxHold,estimatedTargetProbabilityPct:result.test.targetRatePct,estimatedStopProbabilityPct:result.test.stopRatePct,estimatedWinRatePct:result.test.winRatePct,outOfSampleAverageReturnPct:result.test.averageReturnPct,outOfSampleProfitFactor:result.test.profitFactor,ret5Pct:round(f.ret5,2),ret20Pct:round(f.ret20,2),relativeStrength20Pct:round(f.rs20,2),volumeRatio20:round(f.vr,2),rsi14:round(f.r14,1),averageTurnover20Egp:round(f.turnover,0)});}}
  const dedup=new Map();for(const item of opportunities.sort((a,b)=>b.combinedScore-a.combinedScore)){if(!dedup.has(item.ticker))dedup.set(item.ticker,item);}
  const unique=[...dedup.values()];
  const recommendations=unique.filter(x=>!x.extended).slice(0,5).map((x,i)=>({...x,rank:i+1,status:'PRACTICAL_CANDIDATE_PENDING_PRICE_CONFIRMATION',statusAr:'فرصة عملية مشروطة بتأكيد سعر الافتتاح وعدم حدوث فجوة'}));
  const extendedMomentumWatch=unique.filter(x=>x.extended).slice(0,5).map((x,i)=>({rank:i+1,ticker:x.ticker,companyNameAr:x.companyNameAr,strategyId:x.strategyId,strategyLabelAr:x.strategyLabelAr,close:x.close,ret5Pct:x.ret5Pct,ret20Pct:x.ret20Pct,volumeRatio20:x.volumeRatio20,rsi14:x.rsi14,reasonAr:'السهم قوي لكنه ممتد سعريًا؛ الانتظار لإعادة اختبار أو تهدئة أفضل من المطاردة'}));
  const latestRecent=usable.slice(-20);const missed=missedOpportunities(store,latestRecent);
  const practicalReady=Boolean(validatedModels.length&&recommendations.length);
  const decision={schemaVersion:'15.0.0',generatedAt:new Date().toISOString(),sessionDate:latestDate,mode:'FULL_MARKET_CROSS_SECTIONAL_WALK_FORWARD',practicalReady,status:practicalReady?'PRACTICAL_CANDIDATES_AVAILABLE':'NO_VALIDATED_STRATEGY',statusAr:practicalReady?`توجد ${recommendations.length} فرص عملية مشروطة من مسح السوق بالكامل`:'لا توجد استراتيجية اجتازت التحقق والاختبار النهائي؛ لا يتم اختلاق توصيات',selectedModel:selected,validatedModels:validatedModels.map(x=>x.id),recommendations,extendedMomentumWatch,marketScan:{histories:histories.length,symbolsLatest:(store.byDate.get(latestDate)||[]).length,latestDate},guardrails:{fullMarketScan:true,developmentValidationTestSplit:true,futureLeakageForbidden:true,transactionCostsPct:COST_PCT,targetProbabilityMustExceedStopProbability:true,manualOpeningPriceConfirmation:true,automaticOrders:false},missedOpportunityCapture:missed.tradableCaptureRatePct};
  writeJson(OUT_RESEARCH,{schemaVersion:'15.0.0',generatedAt:new Date().toISOString(),sessions:{development:development.length,validation:validation.length,test:test.length},models:modelResults,selection:selected?selected.id:null,validatedModels:validatedModels.map(x=>x.id),selectionRule:'Model must pass both validation and untouched test; target rate must exceed stop rate.',costPct:COST_PCT});
  ...
  console.log(JSON.stringify({latestDate,histories:histories.length,usableSessions:usable.length,selected:selected?.id||null,models:modelResults,recommendations:recommendations.map(x=>x.ticker),missedCapture:missed.tradableCaptureRatePct},null,2));
}
main();
