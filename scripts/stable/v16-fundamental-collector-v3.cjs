...
const PARSER_VERSION = 'V16.3_EXACT_ROWS_UNIT_SAFE_1.0';
...
const USER_AGENT = process.env.EGX_FUNDAMENTAL_USER_AGENT || 'EGX-Pro-Fundamental-Research/16.3';
...
function suffixMultiplier(value) {
  ...
  return suffix === 'K' ? 1e3 : suffix === 'M' ? 1e6 : suffix === 'B' ? 1e9 : suffix === 'T' ? 1e12 : null;
}
  ...
  for (const html of htmlParts) {
    for (const table of parseTables(html)) {
      for (const row of table) {
        ...
        const values = raw.map(baseNumber);
        const numericCount = values.filter(Number.isFinite).length;
        if (!numericCount) continue;
        const existing = rows.get(key);
        if (!existing || numericCount > existing.numericCount) rows.set(key, { label: row[0], raw, values, numericCount });
      }
    }
  }
  return rows;
...
function amountAt(row, index, pageScale) {
  const base = valueAt(row, index);
  if (!Number.isFinite(base)) return null;
  const explicit = suffixMultiplier(row?.raw?.[index]);
  return round(base * (explicit || pageScale || 1), 2);
}
function statisticAt(row, index) {
  const base = valueAt(row, index);
  if (!Number.isFinite(base)) return null;
  return round(base * (suffixMultiplier(row?.raw?.[index]) || 1), 2);
}
...
function parseRecord(ticker, companyNameAr, pages) {
  ...
  const latestRevenue = amountAt(revenue, 0, scale);
  const annualRevenue = amountAt(revenue, 1, scale);
  const priorRevenue = amountAt(revenue, 2, scale);
  const latestNetIncome = amountAt(netIncome, 0, scale);
  const annualNetIncome = amountAt(netIncome, 1, scale);
  const priorNetIncome = amountAt(netIncome, 2, scale);
  const latestOcf = amountAt(operatingCashFlow, 0, scale);
  const latestFcf = amountAt(freeCashFlow, 0, scale);
  const latestCash = amountAt(cash, 0, scale);
  const latestDebt = amountAt(debt, 0, scale);
  const latestMarketCap = statisticAt(marketCap, 0);
  const latestPs = valueAt(ps, 0);
  ...
  const record = {
    ...
    latest: {
      ...
      grossProfit: amountAt(grossProfit, 0, scale),
      operatingIncome: amountAt(operatingIncome, 0, scale),
      netIncome: latestNetIncome,
      eps: round(valueAt(eps, 0), 4),
      cashAndInvestments: latestCash,
      totalDebt: latestDebt,
      netCashDebt: amountAt(netCash, 0, scale),
      operatingCashFlow: latestOcf,
      capitalExpenditures: amountAt(capex, 0, scale),
      freeCashFlow: latestFcf,
      grossMarginPct: round(valueAt(grossMargin, 0), 2),
      operatingMarginPct: round(valueAt(operatingMargin, 0), 2),
      netMarginPct: round(valueAt(profitMargin, 0), 2),
      freeCashFlowMarginPct: round(valueAt(fcfMargin, 0), 2),
      peRatio: round(valueAt(pe, 0), 3),
      priceToFreeCashFlow: round(valueAt(pfcf, 0), 3),
      priceToSales: round(latestPs, 3),
      priceToBook: round(valueAt(pb, 0), 3),
      returnOnEquityPct: round(valueAt(roe, 0), 2),
      returnOnAssetsPct: round(valueAt(roa, 0), 2),
      currentRatio: round(valueAt(currentRatio, 0), 3),
      debtToEquity: round(valueAt(debtEquity, 0), 3),
      dividendYieldPct: round(valueAt(dividendYield, 0), 2),
      marketCap: latestMarketCap,
      enterpriseValue: statisticAt(enterpriseValue, 0),
      sharesOutstanding: statisticAt(sharesOutstanding, 0),
    },
    annual: {
      current: { revenue: annualRevenue, netIncome: annualNetIncome },
      prior: { revenue: priorRevenue, netIncome: priorNetIncome },
    },
    calculated: {
    ...
    parseDiagnostics: {
      ...
      revenueRaw: revenue?.raw?.slice(0, 3) || [],
      marketCapRaw: marketCap?.raw?.[0] || null,
    },
  };
  ...
  if (Number.isFinite(latestMarketCap) && Number.isFinite(latestPs) && latestPs > 0 && latestRevenue > 0) {
    const impliedPs = latestMarketCap / latestRevenue;
    const factor = Math.max(impliedPs / latestPs, latestPs / impliedPs);
    record.parseDiagnostics.priceSalesConsistencyFactor = round(factor, 3);
    if (!Number.isFinite(factor) || factor > 20) anomalies.push('PRICE_SALES_SCALE_MISMATCH');
  }
  for (const [key, value] of Object.entries(record.latest)) {
    if (Number.isFinite(value) && Math.abs(value) > 1e15 && !['sharesOutstanding'].includes(key)) anomalies.push(`EXTREME_${key.toUpperCase()}`);
  }
  record.parseDiagnostics.availableMetricCount = available;
  record.parseDiagnostics.anomalies = [...new Set(anomalies)];
  record.parseDiagnostics.parseAccepted = available >= 10 && record.parseDiagnostics.anomalies.length === 0;
  if (!record.parseDiagnostics.parseAccepted) throw new Error(`FINANCIAL_PARSE_REJECTED_${record.parseDiagnostics.anomalies.join('_') || available}`);
  return record;
}
...
async function main() {
  ...
  for (const [ticker, record] of Object.entries(old.records || {})) {
    if (record?.parseDiagnostics?.parserVersion === PARSER_VERSION && record?.parseDiagnostics?.parseAccepted === true) records[ticker] = record;
  }
  const cache = { records };
  ...
  const recommendationTickers = (decision.recommendations || []).map(r => String(r.ticker || '').toUpperCase());
  const output = {
    schemaVersion: '16.3.0',
    generatedAt: nowIso(),
    parserVersion: PARSER_VERSION,
    provider: {
      name: 'stockanalysis_sp_global_standardized',
      tier: 'SECONDARY_STANDARDIZED',
      officialOverridesSupported: true,
      methodology: 'Exact row matching across income statement, balance sheet, cash flow and statistics. Monetary page scale is applied only when a cell has no explicit K/M/B/T suffix. Growth rows cannot alias monetary rows.',
    },
    ...
    recommendationCoverageCount: recommendationTickers.filter(ticker => records[ticker]).length,
    records,
    failures,
    runFailures,
  };
  writeJson(RAW_PATH, output);
  console.log(JSON.stringify({ parserVersion: PARSER_VERSION, universe: all.length, attempted: selected.length, succeeded, failed: selected.length - succeeded, coverage: values.length, recommendationCoverage: output.recommendationCoverageCount, recommendationTickers }, null, 2));
}
