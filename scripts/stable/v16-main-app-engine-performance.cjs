...
async function main(){
  ...
  const v16Row={
    ...
    evidenceComparable:true,evidenceStatus:'BLOCKED_WALK_FORWARD_TARGET_STOP_AUDIT',evidenceClass:'EXACT_V16_9_WALK_FORWARD',
  ...
  const v19Fetch=await firstJson(V19_URLS);const v19=v19Fetch.value;const v19Registry=engine('V19_CHALLENGER');
  let v19Row;
  if(v19?.schemaVersion==='19.5.0-target-stop-audit-v1'&&v19?.engineId==='V19_CHAT_GPT_NATIVE_CHALLENGER_V6'){
    ...
    v19Row={
      ...
      sessionAligned:v19Registry.sessionAligned===true,evidenceComparable:true,evidenceStatus:'REUSED_HOLDOUT_TARGET_STOP_DIAGNOSTIC',evidenceClass:v19.evidenceClass||'REUSED_V19_V6_HOLDOUT_DIAGNOSTIC',
    ...
    v19Row=noEvidence('V19_V6','V19 V6','PENDING_AUDIT','TARGET_STOP_AUDIT_PENDING',v19Registry.sessionDate||null,{sessionAligned:v19Registry.sessionAligned===true,sourceError:v19Fetch.error||null,noteAr:'تم تجهيز Target/Stop Audit في V19؛ ينتظر اكتمال ونشر ناتج الـ20 جلسة قبل عرض نسبة غير موثقة.'});
  }
  ...
  const v20=engine('V20_NATIVE');
  const v20Row=noEvidence('V20_NATIVE','V20 Native','PENDING_RESOLVED_EVIDENCE','NO_COMPARABLE_RESOLVED_TARGET_STOP_SAMPLE',v20.sessionDate||null,{sessionAligned:v20.sessionAligned===true,noteAr:'لا تُعرض نسبة Target/Stop قبل وجود Forward outcomes مكتملة بنفس عقد القياس.'});
  ...
  const comparable=[v16Row,v19Row].filter(x=>x.evidenceComparable&&Number.isFinite(x.targetRatePct)&&Number.isFinite(x.stopRatePct));
  const leader=comparable.length?comparable.slice().sort((a,b)=>(b.targetRatePct-b.stopRatePct)-(a.targetRatePct-a.stopRatePct))[0]:null;
  const out={
    ...
    sourceHealth:{v19AuditAvailable:v19Row.evidenceComparable===true,v19AuditSource:v19Row.sourceUrl||null,v19AuditError:v19Row.sourceError||null},
    comparableEngineCount:comparable.length,leaderByTargetMinusStop:leader?{id:leader.id,label:leader.label,edgePct:round(leader.targetRatePct-leader.stopRatePct)}:null,
    rows:[v16Row,v19Row,v17Row,v20Row,quantRow]
  };
  fs.mkdirSync(path.dirname(OUT),{recursive:true});fs.writeFileSync(OUT,JSON.stringify(out,null,2)+'\n');
  console.log(JSON.stringify({schemaVersion:out.schemaVersion,sessionDate:out.sessionDate,rows:out.rows.map(x=>({id:x.id,target:x.targetRatePct,stop:x.stopRatePct,status:x.status})),leader:out.leaderByTargetMinusStop},null,2));
}
