...
const minified = Buffer.from(minifiedText);
const packed = zlib.gzipSync(minified, { level: 9 });
const b64 = packed.toString('base64');
const payloadSha256 = crypto.createHash('sha256').update(minified).digest('hex');
...
const loader = `<script>\nwindow.__V18_LIVE_DATA_SOURCE__=${JSON.stringify(liveJsonUrl)};\nwindow.__V18_LIVE_MANIFEST_SOURCE__=${JSON.stringify(liveManifestUrl)};\nwindow.__V18_LIVE_LOADER_VERSION__='github-live-v3';\nwindow.__V18_AUTO_REFRESH_VERSION__='github-live-watch-v3';\nwindow.__V18_AUTO_REFRESH_INTERVAL_MS__=60000;\nwindow.__V18_EMBEDDED_PAYLOAD_SHA256__=${JSON.stringify(payloadSha256)};\nwindow.__V18_EMBEDDED_DATA__=${minifiedText};\nwindow.__V18_PARSE_AND_VALIDATE__=function(live){if(live?.schemaVersion!=='18.2.0-shadow')throw new Error('Live V18 schema mismatch');if(live?.dataHealth?.status!=='PASS'||Number(live?.dataHealth?.criticalFailureCount||0)!==0)throw new Error('Live V18 data-health gate failed');if(!live?.sessionId)throw new Error('Live V18 session is missing');if(!Array.isArray(live?.allCandidates)||live.allCandidates.length<20)throw new Error('Live V18 candidates are incomplete');return live};\nwindow.__V18_MARK_ACTIVE_DATA__=function(live,source,hash){window.__V18_ACTIVE_SESSION__=String(live.sessionId||'');window.__V18_ACTIVE_PAYLOAD_SHA256__=String(hash||'');window.__V18_DATA_SOURCE_USED__=source;return live};\nwindow.__V18_DECODE_GZIP_B64__=async function(encoded){if(typeof DecompressionStream==='undefined')throw new Error('gzip-decode-unsupported');const bin=atob(encoded);const bytes=Uint8Array.from(bin,c=>c.charCodeAt(0));const stream=new Blob([bytes]).stream().pipeThrough(new DecompressionStream('gzip'));return new Response(stream).text()};\nwindow.__V18_EMBEDDED_DATA_LOADER__=async function(){const live=window.__V18_PARSE_AND_VALIDATE__(window.__V18_EMBEDDED_DATA__);return window.__V18_MARK_ACTIVE_DATA__(live,'embedded-json-fallback',window.__V18_EMBEDDED_PAYLOAD_SHA256__)};\nwindow.__V18_DATA_LOADER__=async function(){let manifest=null;try{const controller=typeof AbortController!=='undefined'?new AbortController():null;const timer=controller?setTimeout(()=>controller.abort(),15000):null;try{const mr=await fetch(window.__V18_LIVE_MANIFEST_SOURCE__+'?live='+Date.now(),{cache:'no-store',signal:controller?.signal});if(mr.ok){manifest=await mr.json();if(manifest?.schemaVersion==='18.2.0-shadow'&&manifest?.sessionId&&manifest?.dataHealth==='PASS'&&Number(manifest?.criticalFailureCount||0)===0&&typeof manifest?.liveDataGzipB64==='string'&&manifest.liveDataGzipB64.length>100){try{const text=await window.__V18_DECODE_GZIP_B64__(manifest.liveDataGzipB64);const live=window.__V18_PARSE_AND_VALIDATE__(JSON.parse(text));if(live.sessionId!==manifest.sessionId)throw new Error('Live manifest/data session mismatch');return window.__V18_MARK_ACTIVE_DATA__(live,'github-live-manifest-gzip',manifest.payloadSha256||'')}catch(gzipErr){console.warn('V18 compressed live payload unavailable; falling back to raw JSON.',gzipErr)}}}const r=await fetch(window.__V18_LIVE_DATA_SOURCE__+'?live='+Date.now(),{cache:'no-store',signal:controller?.signal});if(!r.ok)throw new Error('Live V18 JSON HTTP '+r.status);const live=window.__V18_PARSE_AND_VALIDATE__(await r.json());if(manifest?.sessionId&&live.sessionId!==manifest.sessionId)throw new Error('Live raw JSON/manifest session mismatch');return window.__V18_MARK_ACTIVE_DATA__(live,'github-live-json',manifest?.payloadSha256||'')}finally{if(timer)clearTimeout(timer)}}catch(err){console.warn('V18 live source unavailable; using embedded verified JSON snapshot.',err);return window.__V18_EMBEDDED_DATA_LOADER__()}};\nwindow.__V18_CHECK_FOR_NEW_SESSION__=async function(){if(document.visibilityState==='hidden')return;try{const r=await fetch(window.__V18_LIVE_MANIFEST_SOURCE__+'?watch='+Date.now(),{cache:'no-store'});if(!r.ok)return;const m=await r.json();if(m?.schemaVersion!=='18.2.0-shadow'||m?.dataHealth!=='PASS'||Number(m?.criticalFailureCount||0)!==0||!m?.sessionId)return;const activeSession=String(window.__V18_ACTIVE_SESSION__||'');const activeHash=String(window.__V18_ACTIVE_PAYLOAD_SHA256__||'');const targetSession=String(m.sessionId);const targetHash=String(m.payloadSha256||'');const sessionChanged=Boolean(activeSession)&&targetSession!==activeSession;const sameSessionPayloadChanged=Boolean(activeSession)&&targetSession===activeSession&&Boolean(targetHash)&&targetHash!==activeHash;if(!sessionChanged&&!sameSessionPayloadChanged)return;const targetKey=targetSession+':'+(targetHash||'nohash');const key='v18-reload-'+targetKey;const last=Number(sessionStorage.getItem(key)||0);if(Date.now()-last<120000)return;sessionStorage.setItem(key,String(Date.now()));location.reload()}catch(err){console.warn('V18 live-session watch retrying later.',err)}};\nwindow.addEventListener('focus',()=>window.__V18_CHECK_FOR_NEW_SESSION__());\ndocument.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')window.__V18_CHECK_FOR_NEW_SESSION__()});\nsetInterval(()=>window.__V18_CHECK_FOR_NEW_SESSION__(),window.__V18_AUTO_REFRESH_INTERVAL_MS__);\n</script>`;

html = html.replace('<script src="app.js?v=18.2.0"></script>', `${loader}\n<script>\n${app}\n</script>`);
html = html.replace('</body>', '<!-- Rank 1–20 · data-validated Entry Zone / Target 1 / Target 2 / Target 3 / Stop Loss · V18_LIVE_GITHUB_SOURCE_V1 · V18_AUTO_REFRESH_WATCH_V3 · V18_PAYLOAD_HASH_REFRESH -->\n</body>');
...
for(const marker of ['window.__V18_DATA_LOADER__','window.__V18_EMBEDDED_DATA_LOADER__','window.__V18_EMBEDDED_DATA__','window.__V18_CHECK_FOR_NEW_SESSION__','window.__V18_ACTIVE_PAYLOAD_SHA256__','V18_LIVE_GITHUB_SOURCE_V1','V18_AUTO_REFRESH_WATCH_V3','V18_PAYLOAD_HASH_REFRESH']) if(!html.includes(marker)) throw new Error(`Missing ${marker}`);
...
const manifest = {
  ...
  liveLoaderVersion: 'github-live-v3',
  autoRefreshVersion: 'github-live-watch-v3',
  autoRefreshIntervalMs: 60000,
  sameSessionPayloadRefresh: true,
  embeddedVerifiedFallback: true,
  embeddedFallbackEncoding: 'plain-json',
