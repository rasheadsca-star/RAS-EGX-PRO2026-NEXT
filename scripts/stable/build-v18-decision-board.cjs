...
for(const [file,label] of [[seed,'Forward ledger seed'],[engine,'V18 engine'],[extension,'V18.1 leadership extension'],[ranking,'V18.1 evidence ranking'],[performance,'V18.3 engine performance league'],[unified,'V18.2 unified decision center'],[externalPerformance,'External engine comparison adapter'],[externalSnapshots,'External engine snapshots'],[positionPerformance,'V18.3 recommendation position performance'],[multiTarget,'V18 multi-target execution'],[performanceUi,'Performance UI'],[positionUi,'Recommendation position UI']]) if(!fs.existsSync(file))throw new Error(`Missing ${label}`);
require(seed);
require(engine);
require(extension);
require(ranking);
...
if(!data.leadershipSummary||!data.forwardLedgerSummary||!data.dataHealth||!data.enginePerformanceLeague||!data.recommendationPositionPerformance||!data.externalEngineRegistry||!data.executionPlanPolicy)throw new Error('V18 required summaries/performance/execution modules missing');
