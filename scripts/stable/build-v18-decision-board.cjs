...
for(const [file,label] of [[seed,'Forward ledger seed'],[engine,'V18 engine'],[extension,'V18.1 leadership extension'],[ranking,'V18.1 evidence ranking'],[performance,'V18.3 engine performance league'],[unified,'V18.2 unified decision center'],[performanceUi,'Performance UI']]) if(!fs.existsSync(file))throw new Error(`Missing ${label}`);
require(seed);
require(engine);
require(extension);
require(ranking);
...
if(!data.leadershipSummary||!data.forwardLedgerSummary||!data.dataHealth||!data.enginePerformanceLeague)throw new Error('V18 required summaries/performance league missing');
if(data.enginePerformanceLeague.trackingStartsOn!=='2026-09-07')throw new Error('Forward league must start on 2026-09-07');
...
const perfCode=fs.readFileSync(performanceUi,'utf8');let app=fs.readFileSync(appPath,'utf8');if(!app.includes('function renderEnginePerformanceLeague()')){app+='\n\n'+perfCode+'\n';fs.writeFileSync(appPath,app,'utf8');}
