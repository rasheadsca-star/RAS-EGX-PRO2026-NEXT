...
function A(v){return Array.isArray(v)?v:[]};function fail(m){console.error(`V13.17.1 ACCEPTANCE FAILURE: ${m}`);process.exit(1)}
...
if(center.patchVersion!=='13.17.1')fail(`unexpected center patch ${center.patchVersion}`);if(center.liveExecutionEnabled!==false||center.automaticOrderSubmission!==false)fail('live execution safety changed');
...
console.log(`V13.17.1 acceptance passed: searchable=${tickers.length}, outsideToday=${search.summary?.outsideToday||0}, flow=${flow.summary?.analyzedStocks||0}.`);
