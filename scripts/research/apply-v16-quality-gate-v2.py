...
def main():
    ...
    for session in sessions:
        ...
        thresholds = {
            ...
            'requiredRelativeQualityVotes': 3,
            'availableRelativeQualityVotes': 4,
        }
        ...
        for item in broad:
            ...
            vote_count = sum(1 for value in relative_checks.values() if value)
            historical_edge = num(item.get('predictionLiftVsHistoricalBase')) > 1.0
            if historical_edge and vote_count >= 3:
                selected = dict(item)
                selected['qualityChecks'] = {
                    'historicalTop10Edge': historical_edge,
                    **relative_checks,
                    'relativeQualityVoteCount': vote_count,
                    'relativeQualityVotePass': True,
                }
    ...
    method['qualityGateVersion'] = 'v16-quality-gate-v2'
    ...
    method['qualityGate'] = {
        ...
        'mandatoryRule': 'predictionLiftVsHistoricalBase > 1.0',
        'relativeQualityVoteRule': 'At least 3 of 4 same-session relative quality checks must pass.',
        'relativeQualityChecks': [
        ...
        'rationale': 'Corrects V1 over-conjunction and avoids double-counting effectiveModelSupport as a hard gate; no Top-N cap and no threshold fitted to realized returns.',
    }
    doc['schemaVersion'] = 'consensus-v16-quality-gate-walkforward-v2'
