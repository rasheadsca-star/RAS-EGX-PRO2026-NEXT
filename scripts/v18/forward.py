...
def freeze(current, ledger):
    ...
    payload={
        'signalId':f'{session}:V18_SHADOW:V18.0.0',
        'arm':'V18_SHADOW',
        'sessionDate':session,
...
def main():
    ...
    out.write_text(json.dumps(snapshot,ensure_ascii=False,indent=2)+'\n'); print(json.dumps({'snapshot':str(out),'signalId':snapshot['signalId'],'snapshotHash':snapshot['snapshotHash'],'ledgerHash':ledger['ledgerHash'],'portfolioDecision':snapshot['portfolioDecision'],'executionPolicyVersion':EXECUTION_POLICY_VERSION},indent=2))
