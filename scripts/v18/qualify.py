...
def _valid_ohlc(row):
    ...
    return None not in (o, h, l, c) and o > 0 and l > 0 and h >= max(o, c, l) and l <= min(o, c, h)
...
def evaluate_next_session(selection, history, signal_date, transaction_cost_pct=TRANSACTION_COST_PCT):
    ...
    if row is None:
        return {**base, "resolved": False, "executable": False, "state": "PENDING_NEXT_SESSION"}
    ...
    if None in (low, high, stop, target) or not (0 < low <= high and stop < high and target > low):
        return {
            **base,
            "resolved": True,
            "executable": False,
...
def aggregate_member_outcomes(rows):
    ...
    return {
        ...
        "wins": len(wins),
        "losses": len(losses),
        "winRatePct": round(100.0 * len(wins) / len(executable), 4) if executable else None,
        ...
        for selection in entry.get("selections", []):
            ...
            if history is None:
                ...
                outcome = evaluate_next_session(
                    selection, history, entry.get("sessionDate"), transaction_cost_pct
                )
        ...
        cohort_rows.append(
            {
                ...
                "shadowResolved": bool(members) and all(m.get("resolved") for m in members),
...
def _observed_days(entries, cohort_rows):
    if not entries:
        return 0
    starts = [e.get("sessionDate") for e in entries if e.get("sessionDate")]
    outcome_dates = [
        m.get("outcomeDate")
        for c in cohort_rows
        for m in c.get("shadowMembers", [])
...
def _extract_reference(v17_current):
    ...
    return {
        "activeEngine": champion.get("activeEngine") or (v17_current.get("engine") or {}).get("id"),
...
def _paired_status(v18_ledger, v17_ledger, cohort_rows):
    v18_by_session = {
        ...
        if e.get("sessionDate")
    }
    control_by_session = {
        e.get("sessionDate"): e
        for e in v17_ledger.get("entries", [])
        if e.get("engineId") == "V16_9_EQUAL_WEIGHT_BASKET" and e.get("sessionDate")
    }
    common = sorted(set(v18_by_session) & set(control_by_session))
    resolved = []
    pending = []
    for session in common:
        v18_done = bool(v18_by_session[session].get("shadowResolved"))
...
def _data_alignment(current, ledger):
    ...
    hash_match = bool(last and current.get("artifactHash") == last.get("sourceArtifactHash"))
    ...
    selected_dates = {t: (by_ticker.get(t) or {}).get("lastSession") for t in selected}
    ...
    return {
        ...
        "passed": hash_match and selected_aligned,
    }
...
def build_report(root=ROOT):
    ...
    transaction_cost_pct = float(policy.get("transactionCostPct", TRANSACTION_COST_PCT))
    ...
    resolved_promotion_cohorts = sum(1 for c in cohorts if c.get("promotionResolved"))
    ...
    if not alignment.get("passed"):
        blockers.append("FROZEN_SOURCE_SESSION_ALIGNMENT_NOT_PROVEN")
    ...
    blockers += [
        "V18_IS_SHADOW_ONLY",
        "FROZEN_HOLDING_POLICY_NOT_PRESENT_IN_FORWARD_SELECTIONS",
        "FROZEN_PORTFOLIO_ALLOCATION_POLICY_NOT_PRESENT_IN_FORWARD_SELECTIONS",
    ]
    if current:
        model_id = (current.get("model") or {}).get("id")
        rec_versions = sorted(
            {
                r.get("modelVersion")
                for r in current.get("recommendations", [])
                if r.get("modelVersion")
            }
        )
        if model_id and rec_versions and model_id not in rec_versions:
            blockers.append("MODEL_LINEAGE_NAMESPACES_NOT_EXPLICITLY_LINKED")
    blockers = sorted(set(blockers))

    decision = "KEEP_RESEARCH" if blockers else "ELIGIBLE_FOR_PROMOTION_REVIEW"
    return {
        "schemaVersion": "18.0.0-production-qualification-1",
        "engine": "V18",
        "mode": "PRODUCTION_QUALIFICATION",
        "decision": decision,
        "promotionAllowed": decision == "ELIGIBLE_FOR_PROMOTION_REVIEW",
        "policy": policy,
        "methodology": {
            ...
            "shadowBenchmark": "All frozen top-five shadow selections are evaluated for diagnostics only; they cannot promote the engine.",
