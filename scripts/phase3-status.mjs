...
const report={
  ...
  sourceProvenance:{
    ...
    requiredSsotProofs:['CERTIFIED_PRODUCTION_SOURCE_MANIFEST','CERTIFIED_RECONCILIATION_MANIFEST_HASH','PRIMARY_OBSERVATION_CERTIFICATE_HASH','CERTIFIED_PRODUCTION_SESSION_MANIFEST','VERIFIED_BASELINE_AUTHORIZATION_AND_SOURCE_REPORT'],
    downstreamAuthorityGuards:['PRODUCTION_RECOMMENDATION_REQUIRES_CERTIFIED_PRODUCTION_SESSION','PRODUCTION_RECOMMENDATION_REQUIRES_MATCHING_BASELINE_AUTHORIZATION','BASELINE_AUTHORIZATION_SOURCE_REPORT_HASH_RECOMPUTED','BASELINE_AUTHORIZATION_SQL_INSERT_VERIFIED','RESEARCH_RECOMMENDATION_REQUIRES_RESEARCH_SESSION','RECOMMENDATION_LEDGER_AUTHORITY_MODE_MATCH','SESSION_BASELINE_AND_RECOMMENDATION_APPEND_ONLY']
  },
  ...
  interpretation:'Architecture and contracts may pass CI while operational Phase 3 remains FAIL. Identity/disclosure evidence is intentionally insufficient. Phase 4 is machine-locked until an exhaustive receipt-, scope-, and certificate-bound official EGX equity universe, versioned session authority, authoritative post-close current-session observations with independent certified cross-checks, a CERTIFIED_PRODUCTION reconciliation manifest persisted into the SSOT, and a hash-bound CERTIFIED_PRODUCTION Session Manifest over that exact normalized lineage all pass. Any production recommendation additionally requires a verified, append-only Baseline Authorization whose source Phase-3 report hash is recomputed from the report content and whose session, calendar and universe match the Session Manifest; research lineage remains authority-separated.'
};
